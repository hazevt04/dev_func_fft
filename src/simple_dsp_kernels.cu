...
void calc_psds(float* __restrict__ sh_psds, const float* __restrict__ sh_con_sqrs) {
   ...
   if ( A_con_sqr != 0 ) A_psd = 10*__log10f( A_con_sqr ) - TEN_LOG_TEN_FFT_SIZE;
   if ( B_con_sqr != 0 ) B_psd = 10*__log10f( B_con_sqr ) - TEN_LOG_TEN_FFT_SIZE;
   if ( C_con_sqr != 0 ) C_psd = 10*__log10f( C_con_sqr ) - TEN_LOG_TEN_FFT_SIZE;
   if ( D_con_sqr != 0 ) D_psd = 10*__log10f( D_con_sqr ) - TEN_LOG_TEN_FFT_SIZE;
